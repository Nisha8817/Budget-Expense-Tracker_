document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadData() {
        ...
        if (savedState) {
            ...
            state.monthlyBudget = parseFloat(state.monthlyBudget) || 0;
        }
    }
    ...
    function setupEventListeners() {
        ...
        // Monthly budget save
        document.getElementById('save-monthly-budget').addEventListener('click', handleMonthlyBudgetSave);
    }
    ...
    // Handle monthly budget save
    function handleMonthlyBudgetSave() {
        const input = document.getElementById('monthly-budget-input');
        const amount = parseFloat(input.value);

        if (isNaN(amount) || amount < 0) {
            alert('Please enter a valid positive amount for the monthly budget.');
            return;
        }
    ...
    function updateMonthlyBudgetDisplay() {
        const displayElement = document.getElementById('monthly-budget-display');
        if (displayElement) {
            displayElement.textContent = `$${state.monthlyBudget.toFixed(2)}`;
        }
    }

    // Calculate budget prediction for end of month
    function calculateBudgetPrediction() {
        const now = new Date();
        ...
        const currentExpenses = monthlyTransactions
        ...
        const dailySpendingRate = currentExpenses / currentDay;

        // Predict remaining expenses for the month
        const remainingDays = daysInMonth - currentDay;
        const predictedRemainingExpenses = dailySpendingRate * remainingDays;

        // Calculate predicted end-of-month balance
        const predictedBalance = currentIncome - (currentExpenses + predictedRemainingExpenses);
    ...
    function generateAIInsight() {
        ...
        // Calculate daily spending rate
        const dailySpendingRate = currentDay > 0 ? currentExpenses / currentDay : 0;

        // Calculate remaining days and predicted overspend
        const remainingDays = daysInMonth - currentDay;
        const predictedRemainingExpenses = dailySpendingRate * remainingDays;
        const totalPredictedExpenses = currentExpenses + predictedRemainingExpenses;

        // Check if user has set a monthly budget
        let insight = "Analyzing your spending patterns...";

        if (state.monthlyBudget > 0) {
            const budgetDifference = state.monthlyBudget - totalPredictedExpenses;

            if (budgetDifference < 0) {
                // Calculate when they will overspend
                const overspendAmount = Math.abs(budgetDifference);
                const daysToOverspend = Math.ceil(overspendAmount / dailySpendingRate);
                const overspendDate = currentDay + daysToOverspend;

                if (overspendDate <= daysInMonth) {
                    insight = `At this rate, you will overspend your budget by the ${overspendDate}${getOrdinalSuffix(overspendDate)} of the month.`;
                } else {
                    insight = `You're on track to stay within your budget this month. Great job!`;
                }
            } else {
                insight = `You're on track to stay within your budget this month. Keep it up!`;
            }
        } else if (currentIncome > 0 && currentExpenses > 0) {
            const savingsRate = ((currentIncome - currentExpenses) / currentIncome * 100);
            if (savingsRate > 20) {
                insight = `Excellent savings rate of ${savingsRate.toFixed(1)}%! You're managing your finances well.`;
            } else if (savingsRate > 10) {
                insight = `Good savings rate of ${savingsRate.toFixed(1)}%. Consider increasing your emergency fund.`;
            } else if (savingsRate > 0) {
                insight = `Your savings rate is ${savingsRate.toFixed(1)}%. Try to save more for financial security.`;
            } else {
                insight = `You're spending more than you earn. Consider creating a budget to improve your financial health.`;
            }
        } else if (monthlyTransactions.length === 0) {
            insight = "Start tracking your transactions to get personalized financial insights!";
        } else {
            insight = "Continue tracking your expenses to receive more detailed financial insights.";
        }
    ...
    function updateSummaryCards() {
        ...
        // Calculate budget prediction
        const budgetPrediction = calculateBudgetPrediction();
        ...
        document.getElementById('budget-prediction').textContent = `$${budgetPrediction.toFixed(2)}`;
...
// This function fetches the AI report from backend
async function loadAIReport() {
    try {
        const response = await fetch('http://localhost:5000/api/report'); // backend endpoint
        const data = await response.json();

        console.log('AI Report:', data); // just to check in browser console

        // Update your dashboard UI with AI report
        document.getElementById('ai-income').innerText = `₹${data.totalIncome}`;
        document.getElementById('ai-expense').innerText = `₹${data.totalExpense}`;
        document.getElementById('ai-savings').innerText = `₹${data.savings}`;
...
window.addEventListener('DOMContentLoaded', loadAIReport);


function generateAIReport(period = 'weekly') {
    ...
    if (filtered.length === 0) {
      aiText.textContent = "No transactions available for this period.";
      return;
    }
    ...
    const insight = `
      ...
      Highest Expense Category: ${highestCategory} (₹${maxAmount.toLocaleString()}).
      ${totalIncome > totalExpense ? 'Good job! You are saving.' : 'Warning! Expenses exceeded income.'}
    `;
  
    aiText.textContent = insight;
  }
  
  // Load default weekly report on page load
  document.addEventListener('DOMContentLoaded', () => {
    generateAIReport('weekly');
  });
